<?php  
	...
	include "../utiliti/koneksi.php";
		...
		if (isset($_POST['booking_buku'])) {
			
			$query_anggota = mysql_query("SELECT id_anggota FROM t_anggota WHERE nim = '".$_SESSION['nim']."'");
			$fetch_anggota = mysql_fetch_array($query_anggota);

			$id_anggota = $fetch_anggota['id_anggota'];
			$kd_buku    = $data['kd_buku'];
			$tempo = date("Y-m-d", strtotime("+3 day"));

			$query_add_booking = mysql_query("INSERT INTO t_booking(id,id_anggota,kd_buku,tempo) VALUES('','$id_anggota','$kd_buku','$tempo')") or die(mysql_error());

			if ($query_add_booking) {
				mysql_query("UPDATE t_buku SET status = 'sudah dibooking' WHERE kd_buku = '$kd_buku'");
				?>
					<script>
						alert("Transaksi Booking Berhasil !! Anda memiliki waktu 3 hari untuk melakukan transaksi peminjaman ke pustakawan.");
						window.close();
					</script>
				<?php
			}
							...
							<table id="tabel" class="table table-striped">
								<tbody>
									<tr>
										...
										<td align="right">
											<?php  
												$query_cek_booking = mysql_query("SELECT * FROM t_booking JOIN t_anggota ON t_booking.id_anggota = t_anggota.id_anggota WHERE nim = '".$_SESSION['nim']."'");
												$cek_booking = mysql_num_rows($query_cek_booking);

												if ($data['status'] == "sudah dibooking" && $cek > 0) {
													echo '<button type="submit" disabled="" title="Buku sudah di booking" name="booking_buku"><b>Booking Buku ini</b></button>';
												} else {
													?>
													<form action="" method="POST" name="fbooking">
														<button type="submit" name="booking_buku"><b>Booking Buku ini</b></button>
													</form>
												<?php
												}
											?>
												<button onclick="window.close()" class="disabled"><b>Tutup Window</b></button>
										</td>										
									</tr>
									<tr>
